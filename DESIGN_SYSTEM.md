...
| **Grower / twin** (PestRadar grower zoom, `pestradar-app`) | One kas: reconstructed climate on a transferred envelope (one zone per kas), literature life-history parameters, the grower's own weekly cards at 20 kap positions | The **cohort model's emergence curve** with its error band, stage icons, the floor plan with the cards; timing and shape against the cards | No absolute population size; no species (the count is "Trips"); no difference between afdelingen (one zone); never that a peak was predicted before it appeared on the cards; the axis is catch, not population |
